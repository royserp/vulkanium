        ...
        for (ModContainer container : FabricLoader.getInstance().getAllMods()) {
            ...
            if (meta.containsCustomValue(JSON_KEY_SODIUM_OPTIONS)) {
                ...
                if (overrides.getType() != CustomValue.CvType.OBJECT) {
                    System.out.printf("[Vulkanium] Mod '%s' contains invalid Vulkanium option overrides, ignoring", meta.getId());
                    continue;
                }

                for (Map.Entry<String, CustomValue> entry : overrides.getAsObject()) {
                    if (entry.getValue().getType() != CustomValue.CvType.BOOLEAN) {
                        System.out.printf("[Vulkanium] Mod '%s' attempted to override option '%s' with an invalid value, ignoring", meta.getId(), entry.getKey());
                        continue;
                    }
