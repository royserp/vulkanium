# Usage of the Vulkanium Config API

The Vulkanium Config API lets mods add their own pages to the Video Settings screen, which Vulkanium replaces with its own screen.
...
The Vulkanium Config API is intended for mods that add video settings, not as a general purpose config API. For general purpose configuration, use the platform's appropriate mod list and a config library.
...
Vulkanium redirects Minecraft's "Video Settings" screen to its own screen. Historically, third-party mods have mixed into Vulkanium to add buttons to their own settings pages or additional options to Vulkanium's pages.

With this API, these mods will not need to touch Vulkanium's internals anymore and should be able to operate independently of the GUI's implementation details. The API may not be able to cover all use cases where mods mixed into Vulkanium's options code, but it should cover most of the common ones.

Registration of options happens in two stages: Early and late. Early registration happens when Vulkanium initializes its own early options before the window is created. Late registration happens after the game launched. Most mods will only need to use late registration. These stages are independent and only options that are registered in the late stage will show up in the GUI.
...
### Dependency on Vulkanium's API

Vulkanium publishes its api package on a maven repository that you can depend on in your buildscript. It needs `modImplementation` on Fabric 1.21.11, and `implementation` on Fabric 1.21.12+ and NeoForge 1.21.11+.
...
dependencies {
    ...
    modImplementation "net.caffeinemc:vulkanium-fabric-api:0.8.0+mc1.21.11"
    // OR using a snapshot build
    modImplementation "net.caffeinemc:vulkanium-fabric-api:0.8.0-SNAPSHOT+mc1.21.11-pre3-build.773"
}
```
...
dependencies {
    ...
    // using a tagged API build 
    implementation "net.caffeinemc:vulkanium-neoforge-api:0.8.0+mc1.21.11"
    // OR using a snapshot build
    implementation "net.caffeinemc:vulkanium-neoforge-api:0.8.0-SNAPSHOT+mc1.21.11-pre3-build.773"
}
```
...
Entrypoint classes that Vulkanium calls to run your options registration code can be declared either in your mod's metadata file, or on NeoForge with a special annotation.

#### With a Metadata Entry

Metadata-based entrypoints use the key `vulkanium:config_api_user` and the value is the full reference to a class that implements the `net.rs.vulkanium.api.config.ConfigEntryPoint` interface.
...
"vulkanium:config_api_user" = "com.example.examplemod.ExampleModConfigBuilder"
```
...
Vulkanium constructs one instance of the entrypoint class, and then calls the early and late registration methods at the right time.
...
The API is largely self-explanatory and an example is provided above. Also see Vulkanium's own options registration for a more in-depth example of the API's usage.
...
It's a typical use case to want to change one of Vulkanium's or another mod's options to have a different range or behavior. This can be achieved in one of two ways: by replacing the option or by applying an overlay to it.
...
Both of these special dependencies are typically not needed and making use of them is most likely user error. Vulkanium only uses them for making sure the extremely weird GUI Scale option behaves as expected.