...
import net.rs.vulkanium.client.config.structure.IntegerOption;
...
import net.rs.vulkanium.client.gui.options.control.ControlElement;
import net.rs.vulkanium.client.gui.prompt.ScreenPrompt;
import net.rs.vulkanium.client.gui.prompt.ScreenPromptable;
import net.rs.vulkanium.client.gui.screen.ConfigCorruptedScreen;
import net.rs.vulkanium.client.gui.widgets.*;
...
public class VideoSettingsScreen extends Screen implements ScreenPromptable, ScrollableTooltip.TooltipParent {
    ...
    private VideoSettingsScreen(Screen prevScreen, @Nullable OptionPage initiallyFocusedPage) {
        super(Component.literal("Vulkanium Renderer Settings"));
    ...
    private void checkPromptTimers() {
        ...
        var options = VulkaniumClientMod.options();
    ...
    private void openDonationPrompt(VulkaniumOptions options) {
    ...
    public static Screen createScreen(Screen currentScreen, @Nullable OptionPage initiallyFocusedPage) {
        if (VulkaniumClientMod.options().isReadOnly()) {
    ...
    private void rebuild() {
        ...
        if (stackVertically) {
            this.applyButton = new FlatButtonWidget(new Dim2i(this.closeButton.getX(), this.closeButton.getY() - (Layout.INNER_MARGIN + Layout.BUTTON_SHORT), Layout.BUTTON_LONG, Layout.BUTTON_SHORT), Component.translatable("vulkanium.options.buttons.apply"), ConfigManager.CONFIG::applyAllOptions, true, false);
            this.undoButton = new FlatButtonWidget(new Dim2i(this.applyButton.getX(), this.applyButton.getY() - (Layout.INNER_MARGIN + Layout.BUTTON_SHORT), Layout.BUTTON_LONG, Layout.BUTTON_SHORT), Component.translatable("vulkanium.options.buttons.undo"), this::undoChanges, true, false);
        } else {
            this.applyButton = new FlatButtonWidget(new Dim2i(this.closeButton.getX() - Layout.INNER_MARGIN - Layout.BUTTON_LONG, this.getLimitY() - (Layout.INNER_MARGIN + Layout.BUTTON_SHORT), Layout.BUTTON_LONG, Layout.BUTTON_SHORT), Component.translatable("vulkanium.options.buttons.apply"), ConfigManager.CONFIG::applyAllOptions, true, false);
            this.undoButton = new FlatButtonWidget(new Dim2i(this.applyButton.getX() - Layout.INNER_MARGIN - Layout.BUTTON_LONG, this.getLimitY() - (Layout.INNER_MARGIN + Layout.BUTTON_SHORT), Layout.BUTTON_LONG, Layout.BUTTON_SHORT), Component.translatable("vulkanium.options.buttons.undo"), this::undoChanges, true, false);
        }
    ...
    private void hideDonationButton() {
        VulkaniumOptions options = VulkaniumClientMod.options();
    ...
    public void extractRenderState(@NonNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        if (this.prompt != null) {
            ...
            this.tooltip.extractRenderState(graphics);
        }
    }
    ...
    private void openDonationPage() {
        Util.getPlatform().openUri("https://www.curseforge.com/members/theroyalserpant/projects/donate");
    }
    ...
    public boolean mouseScrolled(double x, double y, double f, double amount) {
        ...
        if (Minecraft.getInstance().hasControlDown()) {
            var location = Identifier.parse("vulkanium:general.gui_scale");
    ...
    static {
        DONATION_PROMPT_MESSAGE = List.of(
                ...
                FormattedText.composite(Component.literal("It seems that you've been enjoying "), Component.literal("Vulkanium").withColor(0x27eb92), Component.literal(", the powerful and open rendering optimization mod for Minecraft.")),
