...
public class ResourcePackScanner {
    private static final Logger LOGGER = LoggerFactory.getLogger("Vulkanium-ResourcePackScanner");
    ...
    /**
     * <a href="https://github.com/theroyalserpant/Vulkanium/issues/1569">#1569</a>
    ...
    private static void printToasts(Collection<ScannedResourcePack> resourcePacks) {
        ...
        if (!incompatibleResourcePacks.isEmpty()) {
            showConsoleMessage("vulkanium.console.core_shaders_error", true, MessageLevel.SEVERE);
        ...
        if (!likelyIncompatibleResourcePacks.isEmpty()) {
            showConsoleMessage("vulkanium.console.core_shaders_warn", true, MessageLevel.WARN);
        ...
        if (shown) {
            showConsoleMessage("vulkanium.console.core_shaders_info", true, MessageLevel.INFO);
        }
    }
        ...
        for (var entry : scanResults) {
            ...
            if (!entry.shaderPrograms.isEmpty()) {
                emitProblem(builder,
                        ...
                        "https://github.com/theroyalserpant/Vulkanium/wiki/Resource-Packs",
     ...
     * which will not work with Vulkanium, but that Vulkanium can ignore.
     *
    ...
    private static List<String> determineIgnoredShaders(PackResources resourcePack) {
        ...
        try {
            var meta = resourcePack.getMetadataSection(VulkaniumResourcePackMetadata.SERIALIZER);
            if (meta != null) {
