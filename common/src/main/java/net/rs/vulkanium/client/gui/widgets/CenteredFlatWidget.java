package net.rs.vulkanium.client.gui.widgets;

import net.rs.vulkanium.client.gui.ButtonTheme;
import net.rs.vulkanium.client.gui.ColorTheme;
import net.rs.vulkanium.client.gui.Colors;
import net.rs.vulkanium.client.gui.Layout;
...
public abstract class CenteredFlatWidget extends AbstractWidget {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        if (this.subtitle == null) {
            this.text(graphics, this.truncateToFitWidth(this.label, textOffset), x1 + textOffset, (int) Math.ceil(((y1 + (this.getTextBoxHeight() - this.font.lineHeight) * 0.5f))), textColor);
        } else {
            var center = y1 + this.getTextBoxHeight() * 0.5f;
            this.text(graphics, this.truncateToFitWidth(this.label, textOffset), x1 + textOffset, (int) Math.ceil(center - (this.font.lineHeight + Layout.TEXT_LINE_SPACING * 0.5f)), textColor);
            this.text(graphics, this.truncateToFitWidth(this.subtitle, textOffset), x1 + textOffset, (int) Math.ceil(center + Layout.TEXT_LINE_SPACING * 0.5f), textColor);
        }
