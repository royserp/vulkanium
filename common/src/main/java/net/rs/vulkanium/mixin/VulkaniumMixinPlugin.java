package net.rs.vulkanium.mixin;

import net.rs.vulkanium.client.data.config.MixinConfig;
...
public class VulkaniumMixinPlugin implements IMixinConfigPlugin {
    private static final String MIXIN_PACKAGE_ROOT = "net.rs.vulkanium.mixin.";

    private final Logger logger = LogManager.getLogger("Vulkanium");
    ...
    public void onLoad(String mixinPackage) {
        try {
            this.config = MixinConfig.load(new File("./config/vulkanium-mixins.properties"));
        } catch (Exception e) {
            throw new RuntimeException("Could not load configuration file for Vulkanium", e);
        }
        ...
        if (dependencyResolutionFailed) {
            this.logger.error("Not applying any Vulkanium mixins; dependency resolution has failed.");
        }

        this.logger.info("Loaded configuration file for Vulkanium: {} options available, {} override(s) found",
